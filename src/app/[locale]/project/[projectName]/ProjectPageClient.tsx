...
const ProjectPageClient = ({ deploy }: ProjectPageClientProps) => {
  ...
  const projectsList = projects[deploy] || [];
  const urlsList = urls[deploy] || [];

  const projectIndex = urlsList.findIndex((url) => url.toLowerCase() === (Array.isArray(projectName) ? projectName[0].toLowerCase() : projectName?.toLowerCase()))
